    ...
    def layout_to_form(self, layout, parent=None, attrib=None, level=1):
        ...
        if layout["type"] == "Control":
            ...
            if schema["type"] != "array":
                child = element_to_form(schema_url, schema, label, attrib)
                parent.append(child)
                if 'x-br' in layout:
                    child.append(ET.Element('br'))
...
def element_to_form(schema_url, schema, label, attrib=None):
    attrib = attrib or {}
    ...
    text_attrib.update({
        jsonschema_html_map.get(k): str(schema.get(k)) for k in jsonschema_html_map if schema.get(k)
    })
    log.warning("text_attrib: %r", text_attrib)
    ET.SubElement(
        d,
        "input",
        attrib=text_attrib
...
def render_array(schema_url, schema, label=None):
    ...
    for i in range(2):
        ...
        if "properties" in item_schema:
            for k, property_schema in item_schema["properties"].items():
                ...
                hl.append(element_to_form(property_url, property_schema, label))

        else:
            property_url = f"{schema_url}_{i}"
            hl.append(element_to_form(property_url, item_schema, label))
